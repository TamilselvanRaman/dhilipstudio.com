      ...
      <main className="flex-1 pt-20">
        ...
        <section className="relative py-14 sm:py-20 px-4 sm:px-6 md:px-12 overflow-hidden border-b border-stone-800 text-center bg-stone-950 text-white min-h-[360px] sm:min-h-[420px] flex items-center justify-center">
          ...
          <div className="relative z-10 max-w-4xl mx-auto py-2">
            ...
            {/* Category Filters - Ultra-Clean Glassmorphic Capsule Bar */}
            <div className="relative z-10 mt-8 sm:mt-10 text-center px-2 sm:px-4">
              <div className="inline-flex flex-wrap items-center justify-center gap-1.5 sm:gap-2.5 p-2 sm:p-2.5 bg-black/70 backdrop-blur-2xl rounded-2xl sm:rounded-full border border-amber-300/30 shadow-[0_20px_50px_rgba(0,0,0,0.7)] max-w-full">
                {categories.map((cat) => {
                  const isActive = activeCategory === cat;
                  const iconsMap: { [key: string]: string } = {
                    ALL: "grid_view",
                    WEDDING: "photo_camera",
                    CANDID: "camera",
                    BRAHMIN: "auto_awesome",
                    "PRE-WEDDING": "favorite",
                    MATERNITY: "child_care",
                    NEWBORN: "face",
                    BIRTHDAY: "cake",
                  };
                  const icon = iconsMap[cat] || "photo";

                  return (
                    <button
                      key={cat}
                      onClick={() => setActiveCategory(cat)}
                      className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-full text-[11px] sm:text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center gap-1.5 cursor-pointer shrink-0 ${
                        isActive
                          ? "bg-gradient-to-r from-[#b88c42] via-[#d4af37] to-[#b88c42] text-stone-950 font-extrabold shadow-lg shadow-[#b88c42]/50 scale-105 border border-amber-200/60"
                          : "text-stone-300 hover:text-white hover:bg-white/10"
                      }`}
                    >
                      <span className={`material-symbols-outlined text-sm ${isActive ? "text-stone-950" : "text-amber-400/80"}`}>
                        {icon}
                      </span>
                      <span>{cat}</span>
                    </button>
        ...
        <section className="py-16 px-4 sm:px-6 md:px-12 max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            {filteredItems.map((item, idx) => {
              ...
              return (
                <ScrollReveal key={item.id} variant="fade-up" delay={(idx % 4) * 100} duration={700}>
                  <Link
